	...
	*/
	var ColumnFilterWidget = function( $Container, oDataTableSettings, i, widgets ) {
		...
		if ( 'oColumnFilterWidgets' in oDataTableSettings.oInit ) {
			...
			if ( 'columnDefs' in oDataTableSettings.oInit.oColumnFilterWidgets ) {
				$.each( oDataTableSettings.oInit.oColumnFilterWidgets.columnDefs, function( iIndex, oColumnDef ) {
	...
	*/
	ColumnFilterWidget.prototype.fnDraw = function() {
		...
		if ( widget.asFilters.length === 0 ) {
			...
			// Build the menu
			// Show empty cell instead of &nbsp;.
			var select_title = widget.oColumn.sTitle;
			if ( '&nbsp;' === select_title ) {
				select_title = '';
			}
			widget.$Select.empty().append( $( '<option selected disabled></option>' ).val( '' ).text( select_title ) );
			$.each( aDistinctOptions, function( i, sOption ) {
				var sText;
				sText = $( '<div>' + sOption + '</div>' ).text();
				widget.$Select.append( $( '<option></option>' ).val( sOption ).text( sText ) );
			} );
