...
$grid = (isset($this->atts['grid']) ? $this->atts['grid'] : 2);
?>
    ...
    <?php else: ?>
        ...
        <div class="lsd-col-<?php echo (12/$grid); ?>">
            <a href="<?php echo esc_url(get_term_link($term->term_id)); ?>">
                <?php if(!isset($this->atts['show_icon']) or (isset($this->atts['show_icon']) and $this->atts['show_icon'])): $icon = LSD_Taxonomies::icon($term->term_id); ?>
                    <?php if(trim($icon)): $have_icon = true; ?><div class="lsd-icon"><?php echo $icon; ?></div><?php endif; ?>
                <?php endif; ?>

                <div class="lsd-title<?php if(!$have_icon) echo ' lsd-title-full'; ?>">
                    <?php echo esc_html($term->name); ?>
                    <?php if(isset($this->atts['show_count']) and $this->atts['show_count']): ?>
                        <span class="lsd-count">(<?php echo esc_html($term->count); ?>)</span>
                    <?php endif; ?>
                </div>
            </a>

            <?php if($this->hierarchical and $children = $this->get_terms($term->term_id)): ?>
        ...
        <?php if($i != 1 and $i <= $grid) echo '<div class="lsd-col-transparent lsd-col-'.(12/$grid).'"></div></div>'; ?>
