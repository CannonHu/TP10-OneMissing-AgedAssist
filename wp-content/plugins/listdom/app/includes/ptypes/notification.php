...
class LSD_PTypes_Notification extends LSD_PTypes
{
    ...
    public function filter_columns_content($column_name, $post_id)
    {
        if($column_name == 'hook')
        {
            ...
            echo (isset($hooks[$hook]) ? $hooks[$hook] : 'N/A');
        }
    }
    
    public function register_metaboxes()
    {
        add_meta_box('lsd_metabox_placeholders', esc_html__('Placeholders', 'listdom'), array($this, 'metabox_placeholders'), $this->PT, 'side', 'default');
        add_meta_box('lsd_metabox_recipients', esc_html__('Recipients', 'listdom'), array($this, 'metabox_recipients'), $this->PT, 'side', 'low');
        add_meta_box('lsd_metabox_content', esc_html__('Content', 'listdom'), array($this, 'metabox_content'), $this->PT, 'normal', 'high');
    }

    public function metabox_placeholders($post)
    {
        // Generate output
        include $this->include_html_file('metaboxes/notification/placeholders.php', array('return_path'=>true));
    }

    public function metabox_recipients($post)
    {
        // Generate output
        include $this->include_html_file('metaboxes/notification/recipients.php', array('return_path'=>true));
    }

    public function metabox_content($post)
    {
        // Generate output
        include $this->include_html_file('metaboxes/notification/content.php', array('return_path'=>true));
    }
    ...
    public function save($post_id, $post)
    {
        ...
        // Original Receiver
        $original_to = isset($lsd['original_to']) ? $lsd['original_to'] : 1;
