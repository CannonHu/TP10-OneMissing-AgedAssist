...
class LSD_Taxonomies_Category extends LSD_Taxonomies
{
    ...
    public function init()
    {
        ...
        add_action('add_meta_boxes', array($this, 'register_metaboxes'), 5, 2);

        add_action(LSD_Base::TAX_CATEGORY.'_add_form_fields', array($this, 'add_form'));
        add_action(LSD_Base::TAX_CATEGORY.'_edit_form_fields', array($this, 'edit_form'));
        add_action('created_'.LSD_Base::TAX_CATEGORY, array($this, 'save_metadata'));
        add_action('edited_'.LSD_Base::TAX_CATEGORY, array($this, 'save_metadata'));

        add_filter('manage_edit-'.LSD_Base::TAX_CATEGORY.'_columns', array($this, 'filter_columns'));
        add_filter('manage_'.LSD_Base::TAX_CATEGORY.'_custom_column', array($this, 'filter_columns_content'), 10, 3);
        add_filter('manage_edit-'.LSD_Base::TAX_CATEGORY.'_sortable_columns', array($this, 'filter_sortable_columns'));
    }
    ...
    public function register()
    {
        $args = array(
            ...
            'show_ui'=>true,
            'show_in_rest'=>false,
            'hierarchical'=>($this->isPro() ? true : false),
            'has_archive'=>true,
            'rewrite'=>array('slug'=>LSD_Options::category_slug()),
            'meta_box_cb'=>false,
        );
    ...
    {
        add_meta_box('lsd_metabox_category', esc_html__('Category', 'listdom').' <span class="lsd-required">*</span>', array($this, 'metabox_cateory'), LSD_Base::PTYPE_LISTING, 'side', 'low');
    }

    public function metabox_cateory($post)
    {
        // Primary Category
    ...
    public function add_form()
    {
        ...
        <div class="form-field">
            ...
            <?php echo LSD_Form::iconpicker(array(
                'name'=>'lsd_icon',
                'id'=>'lsd_icon',
                'value'=>'',
            )); ?>
        ...
        <div class="form-field">
            ...
            <?php echo LSD_Form::imagepicker(array(
                'name'=>'lsd_image',
                'id'=>'lsd_image',
                'value'=>'',
            )); ?>
    ...
    public function save_metadata($term_id)
    {
        ...
        $schema = (isset($_POST['lsd_schema']) and trim($_POST['lsd_schema'])) ? sanitize_text_field($_POST['lsd_schema']) : 'https://schema.org/LocalBusiness';
    ...
    public function filter_columns_content($content, $column_name, $term_id)
    {
        switch($column_name)
        {
            case 'image':
                ...
                $content = wp_get_attachment_image(get_term_meta($term_id, 'lsd_image', true), array(50, 50));
                break;
