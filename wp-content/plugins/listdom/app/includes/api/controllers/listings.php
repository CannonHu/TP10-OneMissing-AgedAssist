...
class LSD_API_Controllers_Listings extends LSD_API_Controller
{
    ...
    public function create(WP_REST_Request $request)
    {
        ...
        $tax = (isset($vars['taxonomies']) and is_array($vars['taxonomies'])) ? $vars['taxonomies'] : array();
        ...
        if(!trim($post_title)) return $this->response(array(
        ...
        $message = '';
        if($status == 'pending') $message = esc_html__('The listing submitted. It will publish as soon as possible.', 'listdom');
        elseif($status == 'publish') $message = esc_html__('The listing published.', 'listdom');
    ...
    public function edit(WP_REST_Request $request)
    {
        ...
        if(!current_user_can('edit_post', $id)) return $this->response(array(
    ...
    public function trash(WP_REST_Request $request)
    {
        ...
        if(!current_user_can('delete_post', $id)) return $this->response(array(
    ...
    public function contact(WP_REST_Request $request)
    {
        ...
        // Required
        if(!trim($name) or !trim($email) or !trim($phone) or !trim($message)) return $this->response(array(
            'data' => new WP_Error('400', esc_html__("Name, Email, Phone and Message fields are required!", 'listdom')),
            'status' => 400
        ));

        // Email is not valid
        if(!is_email($email)) return $this->response(array(
            'data' => new WP_Error('400', esc_html__("Email is not valid!", 'listdom')),
            'status' => 400
        ));

        // Trigger Notification
        do_action('lsd_contact_owner', array(
            'name' => $name,
            'email' => $email,
            'phone' => $phone,
            'message' => $message,
    ...
    public function abuse(WP_REST_Request $request)
    {
        ...
        // Trigger Notification
        do_action('lsd_listing_report_abuse', array(
            'name' => $name,
            'email' => $email,
            'phone' => $phone,
            'message' => $message,
