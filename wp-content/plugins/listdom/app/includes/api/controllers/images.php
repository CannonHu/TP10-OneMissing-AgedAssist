...
class LSD_API_Controllers_Images extends LSD_API_Controller
{
    ...
    {
        if(!current_user_can('upload_files')) return $this->response(array(
        ...
        $image = (is_array($vars) and isset($vars['image'])) ? $vars['image'] : array();
        $tmp = isset($image['tmp_name']) ? $image['tmp_name'] : NULL;

        // Image Not Found
        if(!$tmp) return $this->response(array(
        ...
        // Invalid Extension
        if(!in_array($extension, array('png', 'jpg', 'jpeg', 'gif'))) return $this->response(array(
        ...
        // Upload Failed
        if(isset($uploaded['error'])) return $this->response(array(
