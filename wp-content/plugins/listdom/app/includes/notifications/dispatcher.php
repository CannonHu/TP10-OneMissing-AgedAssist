...
if(!class_exists('LSD_Notifications_Dispatcher')):
...
class LSD_Notifications_Dispatcher extends LSD_Notifications
{
    ...
    {
        // Contact Form
        add_action('wp_ajax_lsd_owner_contact', array($this, 'contact'));
        add_action('wp_ajax_nopriv_lsd_owner_contact', array($this, 'contact'));

        // Report Abuse Form
        add_action('wp_ajax_lsd_report_abuse', array($this, 'abuse'));
        add_action('wp_ajax_nopriv_lsd_report_abuse', array($this, 'abuse'));
	}
    ...
    public function contact()
    {
        ...
        $this->response(array('success'=>1, 'message'=>esc_html__("Your message sent successfully.", 'listdom')));
    }
    ...
    public function abuse()
    {
        ...
        $this->response(array('success'=>1, 'message'=>esc_html__("Your report sent successfully.", 'listdom')));
    }
    ...
    public function handle($hook, $nonce_action)
    {
        ...
        // Post ID is not set
        if(!trim($post_id)) $this->response(array('success'=>0, 'message'=>esc_html__("Invalid request!", 'listdom')));

        // Check if security nonce is set
        if(!isset($_POST['_wpnonce'])) $this->response(array('success'=>0, 'message'=>esc_html__("Security nonce is required.", 'listdom')));

        // Verify that the nonce is valid
        if(!wp_verify_nonce(sanitize_text_field($_POST['_wpnonce']), $nonce_action)) $this->response(array('success'=>0, 'message'=>esc_html__("Security nonce is invalid.", 'listdom')));

        // Email is not valid
        if(!is_email($email)) $this->response(array('success'=>0, 'message'=>esc_html__("Invalid email!", 'listdom')));
