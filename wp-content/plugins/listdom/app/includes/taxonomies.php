...
class LSD_Taxonomies extends LSD_Base
{
    ...
    public function init()
    {
        ...
        add_filter('init', array($this, 'redirect'), 999);

        // Listdom Taxonomy Template
        add_filter('template_include', array($this, 'template'), 99);

        // Listdom Archive Content
        foreach($this->taxonomies() as $tax) add_action($tax.'_archive_content', array($this, 'content'));
    }
    ...
    public static function id($term, $taxonomy)
    {
        if(is_array($term))
        {
            ...
            foreach($term as $t)
            {
                ...
                if($term and isset($term->term_id)) $ids[] = (int) $term->term_id;
            }
    ...
    public static function parents($term, $current = array())
    {
        if($term->parent)
        {
            $current[] = $term->parent;
