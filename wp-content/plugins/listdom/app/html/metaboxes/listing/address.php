...
<div class="lsd-metabox">
    ...
    <div class="lsd-object-type-address">
        ...
        <div class="lsd-form-row">
            <div class="lsd-col-6 lsd-pr-2">
                <input type="text" name="lsd[latitude]" id="lsd_object_type_latitude" placeholder="<?php esc_attr_e('Map Center Latitude', 'listdom'); ?>" value="<?php echo esc_attr($latitude); ?>" />
            </div>
            <div class="lsd-col-6 lsd-pl-2">
                <input type="text" name="lsd[longitude]" id="lsd_object_type_longitude" placeholder="<?php esc_attr_e('Map Center Longitude', 'listdom'); ?>" value="<?php echo esc_attr($longitude); ?>" />
            </div>
        </div>
    </div>
    <?php
        // Show Warning of Missing Google Maps API Key
        if(LSD_Map_Provider::def() == LSD_MP_GOOGLE and (!isset($this->settings['googlemaps_api_key']) or (isset($this->settings['googlemaps_api_key']) and !trim($this->settings['googlemaps_api_key'])))) echo LSD_Base::alert(sprintf(esc_html__('You should insert Google Maps API key in %s menu first!', 'listdom'), '<a href="'.admin_url('admin.php?page=listdom-settings').'" target="_blank"><strong>'.esc_html__('settings', 'listdom').'</strong></a>'), 'warning');
    ?>
